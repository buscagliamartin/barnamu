...
ITEM* SEASON3B::CNewUIItemMng::CreateItemByParameters(const ItemCreationParams* parameters)
{
    ...
    // These mounts always carry their skill regardless of whether the server
    // serialised the HasSkill bit in the item packet.
    if (pNewItem->Type == ITEM_DARK_HORSE_ITEM || pNewItem->Type == ITEM_HORN_OF_FENRIR)
        pNewItem->HasSkill = true;
