...
namespace giPetManager
{
        ...
        if ((InvType == 0) || (InvType == 254) || (InvType == 255))
        {
            if ((InvPos == EQUIPMENT_HELPER) || (InvPos == EQUIPMENT_WEAPON_LEFT))
            {
                ...
                else if (pPetinfo->m_dwPetType == PET_TYPE_DARK_HORSE)
                {
                    ...
                    // Grant Earthshake to Dark Lord when the Dark Horse is alive,
                    // but only if the server has not already placed it in the skill
                    // list (to avoid a duplicate entry in the skill bar).
                    if (gCharacterManager.GetBaseClass(Hero->Class) == CLASS_DARK_LORD)
                    {
                        bool alreadyGranted = false;
                        for (int i = 0; i < MAX_SKILLS; ++i)
                        {
                            if (CharacterAttribute->Skill[i] == AT_SKILL_EARTHSHAKE)
                            {
                                alreadyGranted = true;
                                break;
                            }
                        ...
                        if (!alreadyGranted)
                        {
                            CharacterAttribute->Skill[AT_SKILL_EARTHSHAKE] =
                                (pHeroPetInfo->m_wLife > 0) ? AT_SKILL_EARTHSHAKE : AT_SKILL_UNDEFINED;
                        }
                    }
                }
    ...
    void SetPetItemConvert(ITEM* ip, PET_INFO* pPetInfo)
    {
        ...
        {
            // Keep item Durability in sync with horse life so skill checks (e.g. Earthshake) work.
            ip->Durability = static_cast<std::uint8_t>(std::min<std::uint16_t>(pPetInfo->m_wLife, 255u));

            int EarthshakeIndex = -1;
            int DefenceIndex = -1;
            for (int i = 0; i < ip->SpecialNum; ++i)
            {
                if (ip->Special[i] == AT_SKILL_EARTHSHAKE)
                    EarthshakeIndex = i;
                else if (ip->Special[i] == AT_SET_OPTION_IMPROVE_DEFENCE)
                    DefenceIndex = i;
            }

            if (EarthshakeIndex < 0)
            {
                ip->Special[ip->SpecialNum] = AT_SKILL_EARTHSHAKE;
                ip->SpecialValue[ip->SpecialNum] = 0;
                ip->SpecialNum++;
            }

            const auto defenceValue = static_cast<std::uint8_t>((5 + (CharacterAttribute->Dexterity / 20) + pPetInfo->m_wLevel * 2) & 0xFF);
            if (DefenceIndex < 0)
            {
                ip->Special[ip->SpecialNum] = AT_SET_OPTION_IMPROVE_DEFENCE;
                ip->SpecialValue[ip->SpecialNum] = defenceValue;
                ip->SpecialNum++;
            }
            else
            {
                ip->SpecialValue[DefenceIndex] = defenceValue;
            }
        }
    }
    ...
    bool RenderPetItemInfo(int sx, int sy, ITEM* pItem, int iInvenType)
    {
        ...
        {
            pItem->SpecialNum = 0;
            SetPetItemConvert(pItem, pPetInfo);
        ...
        {
            appendLine(TEXT_COLOR_BLUE, false, false, L"Loading...", 0);
        }
