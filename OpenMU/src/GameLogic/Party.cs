...
public sealed class Party : AsyncDisposable
{
    ...
    public async ValueTask DistributeMoneyAfterKillAsync(IAttackable killed, IPartyMember killer, uint amount)
    {
        ...
        try
        {
            ...
            {
                var vipZenBonus = (player.Account?.State == AccountState.Vip
                    || player.Account?.State == AccountState.GameMaster
                    || player.Account?.State == AccountState.GameMasterInvisible) ? 1.30f : 1.0f; // VIP: 13x zen vs normal 10x
    ...
    private static async ValueTask AwardExperienceAsync(Player player, float perLevel, IAttackable killed)
    {
        ...
        var vipMultiplier = isVip
            ? (isAtMaxLevel && isMasterClass ? (20.0f / 15.0f) : (35.0f / 30.0f)) // VIP: 35x exp / 20x master
            : 1.0f;
