...
public class PartyRequestAction
{
    ...
    public async ValueTask HandlePartyRequestAsync(Player player, Player toRequest)
    {
        ...
        if (await toRequest.PlayerState.TryAdvanceToAsync(PlayerState.PartyRequest).ConfigureAwait(false))
        {
            ...
            if (toRequest.PartyAutoMode == PartyAutoMode.AutoAccept)
            {
                await new PartyResponseAction().HandleResponseAsync(toRequest, true).ConfigureAwait(false);
                return;
            }

            if (toRequest.PartyAutoMode == PartyAutoMode.AutoDecline)
            {
                await new PartyResponseAction().HandleResponseAsync(toRequest, false).ConfigureAwait(false);
                return;
            }

            await toRequest.InvokeViewPlugInAsync<IShowPartyRequestPlugIn>(p => p.ShowPartyRequestAsync(player)).ConfigureAwait(false);
