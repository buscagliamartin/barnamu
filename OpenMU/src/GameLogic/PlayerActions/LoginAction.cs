...
public class LoginAction
{
    ...
    private async ValueTask FinishLoginAsync(Player player, string username, Account account)
    {
        ...
        // BarnaMu: VIP con timer. Si el VIP expiro mientras la cuenta estaba offline,
        // la devolvemos a Normal ANTES de aplicar cualquier beneficio VIP.
        if (account.State == AccountState.Vip
            && account.VipExpirationDate is { } vipExpiration
            && vipExpiration <= DateTime.UtcNow)
        {
            ...
            player.Logger.LogInformation("VIP expired for account [{Username}] while offline; reverted to Normal.", username);
        }

        // VIP benefits: extended vault
        if (account.State == AccountState.Vip
            || account.State == AccountState.GameMaster
            || account.State == AccountState.GameMasterInvisible)
        {
