...
public class DefaultDropGenerator : IDropGenerator
{
    ...
    public async ValueTask<(IEnumerable<Item> Items, uint? Money)> GenerateItemDropsAsync(MonsterDefinition monster, int gainedExperience, Player player)
    {
        ...
        uint money = 0;
        var (droppedItems, moneyResult) = this.GenerateDrops(monster, gainedExperience, isVip ? 1.40 : 1.0); // VIP: 7x drop / 4x exe vs normal 5x / 3x
        if (moneyResult > 0)
        {
            money = moneyResult;
        }
    ...
    {
        // Apply dropMultiplier (VIP +40%) uniformly to every group's chance,
        // so the boost works whether totalChance is below or above 1.0.
