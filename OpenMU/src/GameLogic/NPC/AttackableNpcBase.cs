...
public abstract class AttackableNpcBase : NonPlayerCharacter, IAttackable
{
    ...
    private async ValueTask HandleMoneyDropAsync(uint amount, Player killer)
    {
        ...
        // In a party, distribute zen directly among members instead of dropping on the ground
        if (killer.Party is { } killerParty)
        {
            await killerParty.DistributeMoneyAfterKillAsync(this, killer, amount).ConfigureAwait(false);
            return;
        }

        var isVipKiller = killer.Account?.State == AccountState.Vip
            || killer.Account?.State == AccountState.GameMaster
            || killer.Account?.State == AccountState.GameMasterInvisible;
        var zenBonus = isVipKiller ? 1.30 : 1.0; // VIP: 13x zen vs normal 10x
        var droppedMoney = new DroppedMoney((uint)(amount * killer.Attributes![Stats.MoneyAmountRate] * zenBonus), this.Position, this.CurrentMap);
    ...
    {
        // Normalize exp for zen calculation: solo exp includes server rate + VIP exp bonus, party total doesn't.
        // Divide by both to get a neutral base so zen uses its own independent multiplier.
        var isVipSolo = killer.Party is null && (killer.Account?.State == AccountState.Vip
            || killer.Account?.State == AccountState.GameMaster
            || killer.Account?.State == AccountState.GameMasterInvisible);
        var serverRate = killer.GameContext.ExperienceRate > 0 ? killer.GameContext.ExperienceRate : 1.0;
        var vipExpFactor = isVipSolo ? (35.0 / 30.0) : 1.0;
        var moneyExp = killer.Party is null
            ? (int)(exp / (serverRate * vipExpFactor))
            : exp;
        var (generatedItems, droppedMoney) = await this._dropGenerator.GenerateItemDropsAsync(this.Definition, moneyExp, killer).ConfigureAwait(false);
        if (droppedMoney > 0)
        {
