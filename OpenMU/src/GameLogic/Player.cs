...
public class Player : AsyncDisposable, IBucketMapObserver, IAttackable, IAttacker, ITrader, IPartyMember, IRotatable, IHasBucketInformation, ISupportWalk, IMovable, ILoggerOwner<Player>
{
    ...
    /// <summary>
    /// Gets or sets the automatic party request response mode.
    /// </summary>
    public PartyAutoMode PartyAutoMode { get; set; }
    ...
    public async ValueTask<int> AddExpAfterKillAsync(IAttackable killedObject)
    {
        ...
        if (this.Account?.State == AccountState.Vip
            || this.Account?.State == AccountState.GameMaster
            || this.Account?.State == AccountState.GameMasterInvisible)
        {
            experience *= addMasterExperience ? (20.0 / 15.0) : (35.0 / 30.0); // VIP: 35x exp / 20x master
        }
    ...
    private async ValueTask OnPlayerEnteredWorldAsync()
    {
        ...
        await this.InvokeViewPlugInAsync<ICashShopInitializationPlugIn>(p => p.InitializeCashShopAsync()).ConfigureAwait(false);
