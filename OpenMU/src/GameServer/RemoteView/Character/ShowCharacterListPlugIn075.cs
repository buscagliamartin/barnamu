...
public class ShowCharacterListPlugIn075 : IShowCharacterListPlugIn
{
    ...
    private static string GetDisplayName(string characterName, AccountState accountState)
    {
        return accountState switch
        {
            AccountState.GameMaster => $"[GM]{characterName}",
            AccountState.GameMasterInvisible => $"[GM]{characterName}",
            _ => characterName,
        };
    }
    ...
    public async ValueTask ShowCharacterListAsync()
    {
        ...
        var account = this._player.Account;
        var appearanceSerializer = this._player.AppearanceSerializer;
        // 0.75 doesn't support dark lord (number 16) and newer classes yet
        var supportedCharacters = account.Characters.Where(c => c.CharacterClass?.Number < 16).OrderBy(c => c.CharacterSlot).ToList();
        int Write()
        {
            ...
            foreach (var character in supportedCharacters.OrderBy(c => c.CharacterSlot))
            {
                ...
                characterBlock.Name = GetDisplayName(character.Name, account.State);
