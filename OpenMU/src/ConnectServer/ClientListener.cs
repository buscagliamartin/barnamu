...
internal class ClientListener
{
	...
	private async ValueTask OnClientAcceptingAsync(ClientAcceptingEventArgs e)
	{
		...
		if (remoteIp != null)
		{
			...
			{
				var connectionsFromIp = this.Clients.Count(c => c.Address.Equals(remoteIp));
				if (connectionsFromIp >= MaxConnectionsPerIp)
				{
					this._logger.LogInformation("Rejected connection from {0}: max connections per IP reached.", remoteIp);
					e.Cancel = true;
					return;
				}
			}
